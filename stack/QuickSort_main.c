...
/*@ requires  low == 0 && up == length - 1 && low <= low1 && low1 < up1 && up1<= up && \length(arr) == length;
    ensures  low == 0 && up == length - 1 && \length(arr) == length && 
...
*/
void quick_sort(long* const arr, const long low2, const long up2 ) { // 本函数是说明约束变元需要改名的好例子。
	int i;
    if( low2 < up2) {
		i = partition(arr, low2, up2); 
		quick_sort(arr, low2, i - 1); 
		quick_sort(arr, i + 1, up2);
    }
    return;
}
